...
global $db;
$db = new PDO(
    'mysql:host=localhost;dbname=todo;charset=utf8', // host, dbname, and charset
    'root', // username
    ''  // password
);
...
function get_todo_items(){
	...
	try {
		$query = $db->query( 'SELECT * FROM todo_lists WHERE `deleted` IS NULL ORDER BY `done` ASC, `added` ASC' );
		if( $query ){
...
function output_todo_list(){
	echo '<ol id="todo-list">';
		foreach( get_todo_items() as $item ): ?>
			<li<?php if( $item->done !== "0000-00-00 00:00:00" ) echo ' class="done"'; ?>>
				<input class="tick" type="checkbox" value="<?php echo $item->id; ?>" <?php if( $item->done !== "0000-00-00 00:00:00" ) echo ' checked="checked"'; ?> />
...
function ajax_insert_item(){
	if( isset( $_POST['item'] ) ):
		...
		try {
			$insert = $db->prepare( 'INSERT INTO todo_lists ( `item`, `added` ) values ( ?, NOW() )' );
			$insert->execute( $data );
...
function ajax_remove_item(){
	if( isset( $_POST['remove'] ) ):
		...
		try {
			$delete = $db->prepare( 'UPDATE todo_lists SET `deleted` = NOW() WHERE `id` = ?' );
			$delete->execute( $data );
			$response['success'] = true;
		} catch ( PDOException $e ){
			$response['error'] = $e->getMessage();
...
function ajax_tick_item(){
	if( isset( $_POST['tick'] ) ):
		...
		try {
			$tick = $db->prepare( 'UPDATE todo_lists SET `done` = ' . $done . ' WHERE `id` = ?' );
			$tick->execute( $data );
			$response['success'] = true;
		} catch ( PDOException $e ){
			$response['error'] = $e->getMessage();
